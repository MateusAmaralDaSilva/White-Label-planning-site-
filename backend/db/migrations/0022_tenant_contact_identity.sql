-- 0022_tenant_contact_identity.sql - telefone e CNPJ do tenant
--
-- Os campos pertencem a app.tenants porque identificam a empresa/workspace,
-- nao um usuario individual. A API continua sendo a unica camada que aceita
-- esses valores; a role da aplicacao so executa as funcoes administrativas.
...
-- Recria as funcoes SECURITY DEFINER com os dois campos novos.
...
begin
  if not app.is_platform_admin(p_actor_id) then
    raise exception 'usuario % nao e administrador de plataforma', p_actor_id
      using errcode = '42501';
  end if;

  insert into app.tenants
    (id, brand_name, brand_mark, theme_id, brand_logo, plan, paid_until,
     max_users, industry, phone, cnpj)
  values
    (p_tenant_id, p_brand_name, p_brand_mark, p_theme_id, p_brand_logo,
     p_plan, p_paid_until, p_max_users, p_industry, p_phone, p_cnpj);
...
drop function if exists app.admin_update_account(
...
begin
  ...
  if not found then
    raise exception 'tenant % nao encontrado', p_tenant_id using errcode = 'no_data_found';
  end if;
end;
$$;
...
begin
  ...
  return query
    select t.id, t.brand_name, t.brand_mark, t.theme_id, t.brand_logo,
           t.plan, t.paid_until, t.max_users, t.industry, t.phone, t.cnpj,
           (select count(*) from app.users u where u.tenant_id = t.id),
           t.created_at
      from app.tenants t
     where t.id <> 'platform'
...
grant execute on function app.admin_update_account(
